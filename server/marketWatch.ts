...
export const MARKET_WATCH_RESEARCH_CONTEXT = `Research-library framework: classify the market regime and higher-timeframe structure first; incorporate relevant Forex macro drivers or Crypto fundamental and market-microstructure context; confirm execution with live price, volatility, liquidity, and multi-timeframe evidence; define entry, structural invalidation, stop, target, and risk/reward; reject contradictory, stale, or event-distorted setups. Never invent values, never promise profit, and return one BUY or SELL only when the strongest qualified setup is coherent.`;
...
export function buildMarketWatchValidationPrompt(input: { symbol: string; strategy?: string; timeframe?: string; customPrompt?: string; chartContext: unknown; macroContext: unknown; cryptoContext: unknown }): string {
...
export function buildMarketWatchPrompt(input: { symbol: string; strategy?: string; timeframe?: string; customPrompt?: string; chartContext: unknown; marketContext?: unknown }): string {
  ...
  return `You are one member of a unified market-watch panel. Analyze ${input.symbol} using only the supplied live chart context. Strategy: ${input.strategy || "technical analysis"}. Timeframe: ${input.timeframe || "1h"}. User instruction: ${input.customPrompt || "Find the most defensible trade direction."}\n\n${MARKET_WATCH_RESEARCH_CONTEXT}\n${domainContext}\n\nLIVE MACRO / CRYPTO RESEARCH SNAPSHOT (use only as supplied context): ${JSON.stringify(input.marketContext || {})}\n\nLIVE CHART CONTEXT (do not invent values): ${JSON.stringify(input.chartContext)}\n\nReturn strict JSON only with recommendation (BUY or SELL), confidence (0-100), entryPrice, stopLoss, takeProfit, rationale, indicators, and warning. BUY or SELL is required even when evidence is mixed; describe uncertainty in warning. Never guarantee profit.`;
}
